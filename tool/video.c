...
int passCheckAndUpdateProgram(RenderPass *pass) {
	if (g.data_texture->updated) {
		...
		enum {Bytes, CursorX, CursorY} mode = Bytes;
		int x = 0, y = 0;
		int failed = 0;
		for (;;) {
			while (*c && isspace(*c)) ++c;
			...
			int value;
			if (1 != sscanf(c, "%d", &value) || value < 0 || value > 255) {
				MSG("Invalid data at '%s'", c);
				failed = 1;
				break;
			}
			...
			switch (mode) {
			case Bytes:
				data_texture[x + y * DATA_TEX_W * 4] = value;
				++x;
				break;
			case CursorX:
				x = value * 4;
			...
			if (x >= DATA_TEX_W) {
				MSG("WARN: x overflow %d", x);
				x = DATA_TEX_W - 4;
			}
			if (y >= DATA_TEX_H) {
				MSG("WARN: y overflow %d", y);
				y = DATA_TEX_H - 1;
			}
		}
		...
		if (!failed) {
			...
			initTexture(g.textures[Texture_Data], DATA_TEX_W, DATA_TEX_H, GL_RGBA, GL_UNSIGNED_BYTE, data_texture, GL_CLAMP_TO_BORDER);

			FILE *f = fopen("data.inc", "w");
			if (!f) {
				MSG("Failed to open %s", "data.inc");
			} else {
				fprintf(f, "; Autogenerated\n\nDATA_WIDTH EQU %d\nDATA_HEIGHT EQU %d\nsection .ddttex data\ndata_tex:", DATA_TEX_W, DATA_TEX_H);
				for (int i = 0; i < COUNTOF(data_texture); ++i)
					fprintf(f, "%s%d", i % 16 == 0 ? "\n\tdb " : ", ", data_texture[i]);
				fclose(f);
			}
		}
	}
...
void videoInit(int width, int height) {
	...
	g.data_texture = resourceOpenFile("data.txt");
