...
static void serialize(const char *file, const AmData *a) {
	FILE *f = fopen(file, "w");
	if (!f) {
		MSG("Failed to open file '%s' for writing", file);
		return;
	}
	...
	struct {
		...
		struct {
			int tick;
			float v;
		} keypoints[AM_MAX_PROGRAM_OPS];
	} signal_state[AM_MAX_CURSOR_SIGNALS];
	...
	for (int i = 0; i < AM_MAX_PROGRAM_OPS; ++i) {
		...
		case AmOp_Signal_Set: {
			int nsig = op->a.signal_set.signal.value.imm.i;
		...
		case AmOp_Signal_Linear: {
			int nsig = op->a.signal_linear.signal.value.imm.i;
			int nkey = signal_state[nsig].num_keypoints++;

			signal_state[nsig].keypoints[nkey].tick = tick + op->a.signal_linear.ticks.value.imm.i;
			signal_state[nsig].keypoints[nkey].v = op->a.signal_set.value.value.imm.f;
			break;
		}
	...
	fprintf(f, "; Autogenerated, do not modify\n");

	for (int i = 0; i < AM_MAX_CURSOR_SIGNALS; ++i)
	{
		fprintf(f, "sig%d_n EQU %d\n", i, signal_state[i].num_keypoints);
		fprintf(f, "section .dsig%dt data align=1\nsig%d_t:\n", i, i);
		int tick = 0;
		for (int j = 0; j < signal_state[i].num_keypoints; ++j) {
			int dt = signal_state[i].keypoints[j].tick - tick;
			if (dt > 255 || dt < 0) {
				MSG("signal %d, pos %d, dt %d is invalid", i, j, dt);
				goto exit;
			}
			fprintf(f, "\tdb %d\n", dt);
		}
		fprintf(f, "section .dsig%dv data align=1\nsig%d_v:\n", i, i);
		//float value = 0.;
		//const double range = 32.;
		for (int j = 0; j < signal_state[i].num_keypoints; ++j) {
			//int value = (signal_state[i].keypoints[j].v + range) * 255. / (range * 2.);
			int value = signal_state[i].keypoints[j].v * 4.;
			if (value < -127 || value > 126) {
				MSG("signal %d, pos %d, value %d (%f) is invalid", i, j, value, signal_state[i].keypoints[j].v);
				goto exit;
			}
			fprintf(f, "\tdb %d\n", value);
		}
	...
	for (int i = 0; i < AM_MAX_PROGRAM_OPS; ++i) {
		...
		switch (op->type) {
		case AmOp_Wait:
			out_dt = op->a.wait.ticks.value.imm.i - tick;
			tick = op->a.wait.ticks.value.imm.i;
			break;

		case AmOp_Signal_Set: {
			out_sig = op->a.signal_set.signal.value.imm.i;
			const float op->a.signal_set.value.value.imm.f;
			break;
		}
		case AmOp_Signal_Linear: {
			int nsig = op->a.signal_set.signal.value.imm.i;
			int nkey = signal_state[nsig].num_keypoints;
			const float val = signal_state[nsig].keypoints[nkey].k * tick
				+ signal_state[nsig].keypoints[nkey].b;
			float newval = op->a.signal_set.value.value.imm.f;
			if (tick != signal_state[nsig].keypoints[nkey].tick)
				nkey = ++signal_state[nsig].num_keypoints;

			signal_state[nsig].keypoints[nkey].k = (newval - val) / ;
			signal_state[nsig].keypoints[nkey].b = val;
			op->a.signal_set.value.value.imm.f;
			break;
		}
...
void timelineCheckUpdate() {
	...
	serialize("timeline.inc", &data.data);
