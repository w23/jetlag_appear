...
const char *main_shader_glsl =
 ...
 "}"
 "const float x=6.36e+06,z=6.38e+06;"
 "const int i=128,m=16;"
 "const float s=.76,e=s*s,t=8000.,l=1200.;"
 "float r=10.;"
 "const vec3 w=vec3(0.,-x,0.),n=vec3(5.8e-06,1.35e-05,3.31e-05),d=vec3(2e-05),a=d*1.1;"
 "vec4 c,p;"
 "vec3 u;"
 "float g,o;"
 "float h(vec3 v)"
 "{"
   "float i=v.y-400.,y=length(v.xz);"
   "v/=100.;"
   "vec2 x=floor(v.xz),V=x;"
   "float L=10.,z=10.;"
   "for(float b=-1.;b<=1.;b+=1.)"
     "for(float e=-1.;e<=1.;e+=1.)"
       "{"
         "vec2 m=x+vec2(b,e),Z=m+.5,n=floor(m/10.);"
         "p=f(n);"
         "float X=dot(m,m);"
         "c=f(m);"
         "Z+=(c.yz-.5)*smoothstep(100.,200.,X)*p.x;"
         ...
         "else"
           " if(s<z)"
             "z=s;"
       "}"
   "float m=z-L,X=dot(V,V);"
   "c=f(V);"
   "p=f(V/10.);"
   "u=v-vec3(V.x,0.,V.y);"
   "o=step(.8,p.w);"
   "float s=smoothstep(10000.,1000.,y)*(1.-o),b=s*(1.+4.*smoothstep(.8,1.,p.x));"
   "g=(.3-m)*.5;"
 ...
 "vec3 C(vec3 v)"
 "{"
   "return normalize(vec3(h(v+y.yxx),h(v+y.xyx),h(v+y.xxy))-h(v));"
 "}"
 "float C(vec3 v,vec3 i,float f,float z)"
 "{"
   "float y=10.,m=f;"
   "for(int e=0;e<199;++e)"
     "{"
 ...
 "vec2 Q(vec3 i)"
 "{"
   "float m=max(0.,length(i-w)-x);"
   "vec2 y=vec2(exp(-m/t),exp(-m/l));"
   "const float f=5000.;"
   "if(f<m&&m<10000.)"
     "y.y+=step(length(i),50000.)*R(i+vec3(23175.7,0.,v*30.))*max(0.,sin(3.1415*(m-f)/f));"
   "return y;"
 "}"
 "float C(in vec3 v,in vec3 m,in float f)"
 "{"
   "vec3 i=v-w;"
   "float s=dot(i,m),y=dot(i,i)-f*f,V=s*s-y;"
   "if(V<0.)"
     "return-1.;"
   "float x=sqrt(V),L=-s-x,z=-s+x;"
   "return L>=0.?L:z;"
 "}"
 ...
 "{"
   ...
   "for(int V=0;V<m;++V)"
     "{"
       "vec3 z=v+i*float(V)*y;"
       "s+=Q(z)*y;"
     "}"
   "return exp(-(n*s.x+a*s.y));"
 "}"
 "vec3 Q(vec3 v,vec3 f,float x,vec3 y)"
 "{"
   "vec2 c=vec2(0.,0.);"
 ...
 "{"
   ...
   "vec3 m=vec3(.3*(f(vec2(v/16.)).x-.5),1.,0.);"
   "if(v<48.)"
     "r=10.*v/48.,V.y=.001,y=vec3(10000.,15000.,-10000.);"
   "else"
     " if(v<64.)"
       "V.y=.001,y=vec3(-10000.,10000.,-10000.);"
     "else"
       " if(v<128.)"
         "{"
           "b.y=1000.;"
       ...
       "else"
         ...
         "else"
           "{"
             "float U=(v-144.)/64.;"
             "b=vec3(mod(U*2.,10000.)-500.,1000.-200.*U,mod(U*10.,10000.)-5000.);"
             "y.y=4000.*U;"
             "V.y=.01+.5*U;"
   ...
   "V=normalize(V);"
   "L=normalize(b-y);"
   "vec3 x=normalize(cross(normalize(m),L));"
