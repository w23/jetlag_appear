...
void main() {
	...
	vec2 uv = gl_FragCoord.xy/vec2(0.000,0.000)* 2. - 1.; uv.x *= vec2(0.000,0.000).x / vec2(0.000,0.000).y;

	if (gl_FragCoord.y < 10.) {
		gl_FragColor = vec4(vec3(step(gl_FragCoord.x / vec2(0.000,0.000).x, t / 208.)), 1.);
		return;
	}
